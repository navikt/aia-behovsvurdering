import React, { useEffect, useState } from 'react';
import { BodyShort, Link, Modal } from '@navikt/ds-react';
import { ChevronLeftIcon } from '@navikt/aksel-icons';
import Steg1 from './steg1';
import Steg2 from './steg2';
import Steg3 from './steg3';

import { OpplysningerOmArbeidssoker } from '@navikt/arbeidssokerregisteret-utils';
import { Sprak } from '../../contexts/sprak';
import { OnOppdaterOpplysninger } from '../../types/oppdater-opplysninger';

interface PermittertModalProps {
    openModal: boolean;
    setOpenModal: React.Dispatch<React.SetStateAction<boolean>>;
    opplysninger: OpplysningerOmArbeidssoker;
    sprak: Sprak;
    onOppdaterOpplysninger: OnOppdaterOpplysninger;
}

const PermittertModal = (props: PermittertModalProps) => {
    const { openModal, setOpenModal, opplysninger, sprak, onOppdaterOpplysninger } = props;
    const [aktivSide, settAktivSide] = React.useState<number>(1);
    const [valgtSituasjon, settValgtSituasjon] = useState<any>('-1');
    const [tilleggsData, settTilleggsData] = useState<any>(null);
    const opprinneligSituasjon = undefined; // TODO besvarelse?.dinSituasjon?.verdi;

    useEffect(() => {
        settAktivSide(1);
    }, [openModal]);

    const Innhold = () => {
        if (aktivSide === 1) {
            return (
                <Steg1
                    valgtSituasjon={valgtSituasjon}
                    opprinneligSituasjon={opprinneligSituasjon}
                    settValgtSituasjon={settValgtSituasjon}
                    onClick={() => settAktivSide(2)}
                    sprak={sprak}
                />
            );
        }

        if (aktivSide === 2) {
            return (
                <Steg2
                    valgtSituasjon={valgtSituasjon}
                    settValgtSituasjon={settValgtSituasjon}
                    settTilleggsData={settTilleggsData}
                    opprinneligSituasjon={opprinneligSituasjon}
                    onClick={() => settAktivSide(3)}
                    sprak={sprak}
                    erOpprettOppgaveToggletPaa={true} // TODO: bruk feature toggling
                    opplysningerOmArbeidssoker={opplysninger}
                    onOppdaterOpplysninger={onOppdaterOpplysninger}
                />
            );
        }

        if (aktivSide === 3) {
            return (
                <Steg3
                    valgtSituasjon={valgtSituasjon}
                    tilleggsData={tilleggsData}
                    onClose={() => setOpenModal(false)}
                    sprak={sprak}
                />
            );
        }

        return null;
    };

    const Tilbake = () => {
        if (aktivSide !== 2) {
            return null;
        }
        // eslint-disable-next-line react/jsx-no-undef
        return (
            <Link
                href={'/#'}
                onClick={(event) => {
                    event.preventDefault();
                    settAktivSide(aktivSide - 1);
                }}
                className={'py-2'}
            >
                <ChevronLeftIcon /> Tilbake
            </Link>
        );
    };

    return (
        <Modal
            width={'medium'}
            open={openModal}
            onClose={() => setOpenModal(false)}
            header={{ heading: 'Jobbsituasjonen min har endret seg', size: 'medium' }}
        >
            <Modal.Body>
                <Tilbake />
                <BodyShort spacing>
                    <small>Steg {aktivSide}/3</small>
                </BodyShort>
                <Innhold />
            </Modal.Body>
        </Modal>
    );
};

export default PermittertModal;
