...
export interface Steg2Props {
    ...
    settTilleggsData: React.Dispatch<React.SetStateAction<TilleggsdataType>>;
    onClick: () => void;
    sprak: Sprak;
...
const OpplysningeneBrukesTil = () => {
    return <BodyShort className={'mb-4'}>NAV bruker opplysningene for å kunne tilpasse veiledningen din</BodyShort>;
};
...
function genererOppgaveBeskrivelse(
    ...
    if (situasjonerMedOppgaver.includes(valgtSituasjon)) {
        return genererDialogTekst(valgtSituasjon, opprinneligSituasjon, sprak, tilleggsData, merOmSituasjonenMin).tekst;
    }
...
function genererDialogTekst(
    ...
    const svarTekst = lagHentTekstForSprak(SPORSMAL_TEKSTER, sprak);
    ...
    tekstArray.push(`Jobbsituasjonen er endret til "${svarTekst(valgtSituasjon)}".`);
    ...
    if (tilleggsData) {
        ...
        if (valgtSituasjon === PermittertSvar.OPPSIGELSE) {
            tekstArray.push(
                `Oppsigelsen ble mottatt ${oppsigelseDato ? prettyPrintDato(oppsigelseDato as string) : 'på ikke oppgitt dato'}`,
            );
            tekstArray.push(
                `Siste dag med lønn fra arbeidsgiver er ${
                    sisteArbeidsdagDato ? prettyPrintDato(sisteArbeidsdagDato as string) : 'er ukjent'
                }`,
            );
            if (harNyJobb) {
            ...
            tekstArray.push(
                ...
                } fra ${gjelderFraDato ? prettyPrintDato(gjelderFraDato as string) : 'ikke oppgitt dato'}`,
            );
            ...
            tekstArray.push(
                `Jeg leverte oppsigelsen ${oppsigelseDato ? prettyPrintDato(oppsigelseDato as string) : 'på ikke oppgitt dato'}`,
            );
            tekstArray.push(
            ...
            if (harNyJobb && harNyJobb === 'ja' && forsteArbeidsdagDato) {
                tekstArray.push(
                    `Min første arbeidsdag i ny jobb er ${prettyPrintDato(forsteArbeidsdagDato as string)}`,
                );
            }
            ...
            tekstArray.push(
                `Endringen gjelder fra ${gjelderFraDato ? prettyPrintDato(gjelderFraDato as string) : 'ikke oppgitt dato'}`,
            );
        }

        if (opprinneligSituasjon) {
            tekstArray.push(`Jobbsituasjonen er endret fra "${svarTekst(opprinneligSituasjon) ?? '[ukjent]'}".`);
        }
...
function useLagreEndringer(props: Steg2Props) {
    ...
    const { onClick, settTilleggsData, sprak, onOppdaterOpplysninger } = props;
    ...
    const handleLagreEndringer = async (
        ...
        if (tilleggsData) {
            Object.keys(tilleggsData).forEach((key) => {
                const data = tilleggsData[key as keyof TilleggsdataType];
                if (data instanceof Date) {
                    tilleggsData[key as keyof TilleggsdataType] = new Date(
        ...
        try {
            ...
            await onOppdaterOpplysninger(payload, props.erOpprettOppgaveToggletPaa, props.opplysningerOmArbeidssoker);
            settTilleggsData(tilleggsData!);
            onClick();
        } catch (error: any) {
            settFeil(`Feil i handleLagreEndringer: ${error.message}`);
        } finally {
            setLoading(false);
        }
    };
...
const Steg2Wrapper = (props: WrapperProps) => {
    const svarTekst = lagHentTekstForSprak(SPORSMAL_TEKSTER, props.sprak);
    return (
        <>
            <BodyShort>
                <b>Valgt situasjon</b>
            </BodyShort>
            <BodyShort className={'mb-4'}>{svarTekst(props.valgtSituasjon)}</BodyShort>
            {props.children}
        </>
    );
};
...
const TILBAKE_TIL_JOBB = (props: Steg2Props) => {
    ...
    return (
        <Steg2Wrapper valgtSituasjon={props.valgtSituasjon} sprak={props.sprak}>
            <>
...
const KONKURS = (props: Steg2Props) => {
    ...
    return (
        <Steg2Wrapper valgtSituasjon={props.valgtSituasjon} sprak={props.sprak}>
            <>
                <DatePicker {...sisteArbeidsdagProps} strategy="fixed">
                    <DatePicker.Input {...sisteArbeidsdagInput} className={'mb-4'} label="Når er siste arbeidsdag?" />
                </DatePicker>
            ...
            <>
                <DatePicker {...oppsigelseProps} strategy="fixed">
                    <DatePicker.Input
                        {...oppsigelseInput}
                        className={'mb-4'}
                        label={<div className={'flex'}>Fra hvilken dato har du sagt opp stillingen din?</div>}
                    />
                </DatePicker>
                    ...
                    <DatePicker.Input
                        ...
                        label={
                            <div className={'flex'}>
                                ...
                                <HelpText className={'ml-2'}>Siste dag i oppsigelsestiden.</HelpText>
                            </div>
                        }
                    />
                </DatePicker>
            ...
            <>
                <DatePicker {...gjelderFraProps} strategy="fixed">
                    <DatePicker.Input
                        {...gjelderFraInput}
                        className={'mb-4'}
                        label={<div className={'flex'}>Når gjelder endringen fra?</div>}
                    />
                </DatePicker>
                <Textarea
...
const Steg2 = (props: Steg2Props) => {
    ...
    if (valgtSituasjon === PermittertSvar.OPPSIGELSE) {
        ...
        valgtSituasjon === DinSituasjon.ER_PERMITTERT
    ) {
